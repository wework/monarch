...
describe('#getReferenceComponent', () => {
  ...
  test('without any validatorFuncName, required', () => {
    expect(getReferenceComponent({ raw: `customPropType(${ReferencedComponent}).isRequired` })).toEqual(
      ReferencedComponent
    );
  });

  test('with childType', () => {
    expect(getReferenceComponent({ raw: `childType(${ReferencedComponent})` }, 'childType')).toEqual(
      ReferencedComponent
    );
  });

  test('with childType, required', () => {
    expect(getReferenceComponent({ raw: `childType(${ReferencedComponent}).isRequired` }, 'childType')).toEqual(
      ReferencedComponent
    );
  });
});
