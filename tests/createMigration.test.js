...
describe('#createField', () => {
  ...
  test('short text field with required', () => {
    expect(createField({ ...dummyField, required: true }).replace(/\s/g, '')).toBe(
      `.createField('${dummyField.id}').name('${dummyField.name}').type('${dummyField.type}').required(true)`.replace(
        /\s/g,
        ''
      )
    );
  ...
  test('short text field with validations', () => {
    ...
    expect(createField({ ...dummyField, validations }).replace(/\s/g, '')).toBe(
      `.createField('${dummyField.id}').name('${dummyField.name}').type('${
        dummyField.type
      }').validations(${JSON.stringify(validations)})`.replace(/\s/g, '')
    );
  });
  ...
  test('Reference field', () => {
    ...
    expect(createField({ ...dummyField, type, linkType, validations }).replace(/\s/g, '')).toBe(
      `.createField('${dummyField.id}').name('${dummyField.name}').type('${type}').validations(${JSON.stringify(
        validations
      )}).linkType('${linkType}')`.replace(/\s/g, '')
    );
  });
  ...
  test('Array field with items', () => {
    ...
    expect(createField({ ...dummyField, type, items }).replace(/\s/g, '')).toBe(
      `.createField('${dummyField.id}').name('${dummyField.name}').type('${type}').items(${JSON.stringify(
        items
      )})`.replace(/\s/g, '')
    );
  });
  ...
  test('Asset field', () => {
    ...
    expect(createField({ ...dummyField, type, linkType }).replace(/\s/g, '')).toBe(
      `.createField('${dummyField.id}').name('${dummyField.name}').type('${type}').linkType('${linkType}')`.replace(
        /\s/g,
        ''
      )
    );
...
describe('#createFields', () => {
  test('returns all fields with appended contentType', async () => {
    expect(createFields(dummyComponent).replace(/\s/g, '')).toBe(
      `
      ${dummyComponent.content_type}.createField('${dummyField.id}').name('${dummyField.name}').type('${
        dummyField.type
      }')
      ${dummyComponent.content_type}.createField('${dummyFieldTwo.id}').name('${dummyFieldTwo.name}').type('${
        dummyFieldTwo.type
      }').required(true)`.replace(/\s/g, '')
    );
  });
});
